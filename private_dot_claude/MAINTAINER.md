...
- **The file is never the whole mechanism for bright lines.** The Boundaries table's never rows are enforced by the
  existing git guards in `settings.json`. Everything else in the file works by conditioning alone, and its adherence
  is measured in artifacts (reply text, PR bodies, worktree paths), never by asking the model.
...
| Schema over prose | Format-following persists across long contexts far better than prose admonition. This is the one in-file lever the evidence ranks above reminders for reply-time rules, so the section that fails most is carried by a literal reply skeleton, not by its bullets. | The reply skeleton at the end of Talking to the user, the brief's six numbered fields, the three delegate functions, the five-step report procedure, the first-line verdict with its label, the routing and access tables |
...
- **Talking to the user names its handles and makes the verdict checkable.** The harness's own prose telling the
  ...
  four-token correction can retrieve. The reply skeleton at the end of the section is what carries it. A shape the
  model starts generating in is self-reinforcing, where a rule about tone is not. The first line of a reply is the
  checkable artifact, and the handles are how the user corrects a drifted session in a few words. Its diagnosis
  unifies the two observed failures, wall-of-text dumps and opaque coined shorthand, as one error, writing for the
  writer's retrieval machinery instead of the reader's. "Wedges its watch" stays verbatim as the anti-exemplar per the
  anecdote policy.
...
- **The document follows Deliverables are a standing record.** Present state only, no history, nothing a capable
  model already knows, gotchas kept precisely because they are hard-won (the cgroup memory breakdown). The template's
  own punctuation is left as written; the prose rule governs what Claude writes, and re-punctuating the contract would
  bury every substantive change in a diff of rewrapped lines.
...
5. **Positive versus negative framing.** Re-read the vendor's current phrasing guidance and compare against the two
...
- Verify with the CI-faithful render (`sed 's|"darwin"|"linux"|g'` then `chezmoi execute-template --source=.`), run
  `markdownlint-cli2` with the repo config on the rendered output, and read the rendered output once as a cold reader.
- A rule that must hold every time is a hook or a setting, not a sentence. The file still keeps the sentence,
  because it states the standard the guard enforces.
